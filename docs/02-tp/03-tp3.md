...
A faire pour prof:
- **Quoi faire avec le email confirmation?** Le plus simple c'est de le disabler complètement (a voir comment l'app se comporte)
- **BIEN CLARIFIER CE QU`IL FAUT FAIRE AVEC IdentityUser vs ApplicationUser**
...
Prenez d'abord une minute pour utiliser l'application dans son état actuel et voir que vous pouvez faire:
- Voir les **sujets** (**Boards**) existants et voir les **messages** de chaque **sujet**.
- Ajouter un nouveau **sujet**
- Ajouter un nouveau **message** à l'intérieur d'un **sujet**

On utilise le mot **usager** pour référer à une personne qui utilise l'application.
On utilise le mot **utilisateur** pour référer à un **usager** qui est connecté avec un **User**.
...
- Ajoutez une classe **ApplicationUser** qui doit hériter de **IdentityUser**
...
- Note: Il y aura éventuellement un peu plus de travail à faire dans les tests pour supporter l'ajout de la vérification des **utilisateurs** bannis. (Pas besoin d'ajouter des tests en plus, simplement de vérifier que les 2 tests sont toujours fonctionnels)
...
| Tâche | Nb Points |
| :--- | :----: |
| Mettre en place Identity et générez les pages Razor Identity | 2 |
| Créer les liens avec ApplicationUser et faire un Seed | 3 |
| Tests unitaires | 2 |
| Ajoutez le hard delete | 1 |
| Ajoutez l'option de cacher un message | 2 |
| Ajoutez une vue qui permet de voir les utilisateurs | 3 |
| Ajoutez une vue qui affiche les détails d'un utilisateur | 3 |
