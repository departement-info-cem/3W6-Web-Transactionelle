...
## Lien GitHub vers les fichiers de démarrage
-  🔗[fichiers de démarrage du TP2](https://github.com/A26ProgWebTrans/TP2)

--- 

## Consignes
 - L'utilisation de l'IAG pour créer partiellement ou totalement des éléments du travail entraînera automatiquement la note de zéro et un plagiat
...
## Création du projet
1. Créez un dépôt GitHub **PRIVÉ** portant le nom `3W6_TP2_NOM_PRENOM` et ajoutez votre enseignant comme collaborateur
...
:::info
 Les paquets `NuGet.Packaging` et `NuGet.Protocol` utilisés dans le paquet **Microsoft.VisualStudio.Web.CodeGeneration.Design** ont des **vulnérabilités importantes** et **empêchent** le bon fonctionnement de la solution. Il est donc important de mettre manuellement les deux paquets afin de pallier temporairement le problème.
:::
...
2. Remplacez le **contenu** du fichier **d’index** de votre projet `/Views/Home/Index.cshtml` par celui qui est dans le fichier `index.txt`.
...
## Configuration de la base de données et du service SQL Server
...
2.	À partir du diagramme ci-dessous, ajoutez les champs et les annotations nécessaires à votre classe Service. 
...
:::info
Si l’on clique sur une recette (ex. : Couscous royal), nous serons redirigés vers la page d’édition de cette recette.
:::
...
6. Il **faut tenir compte de** la casse : *Orientale* et *orientale* déclenchent le message d’erreur.
- **Résultat attendu :**
...
## LINQ : requêtes (faites d’abord la section « Mise en place »)
...
:::info
Vous devez utiliser au minimum les méthodes LINQ qui sont spécifiées dans les instructions.
:::

6. Informations à afficher :
    - Le nombre de **recettes avec 3 aliments**.
    - L’aliment le plus calorique.
    - Une **liste** des **quatre recettes** dont **la durée totale est la plus courte** (*préparation et cuisson*). Vous devez afficher le nom de la recette et le temps total.
...
## Affichage d’un menu de recettes avec un accordéon Bootstrap
...
6. Le ViewModel RecetteCategorieVM doit, en plus de ses autres propriétés, contenir une `List<RecetteInfoVM>`.
...
| Affichage du menu de recettes avec l’accordéon Bootstrap | 8 |
