...
2. Il y a plusieurs façons possibles de résoudre ce problème, mais c'est une bonne idée d'ajouter un attribut **class** sur le **\<td\>** ou le **\<a\>** qui contient le **nom du trainer** afin de pouvoir facilement les obtenir dans votre code JavaScript.
3. Le surlignage doit se mettre à jour chaque fois que le contenu du champ texte est modifié.
...
4. Ajoutez le code javascript nécessaire pour filtrer directement sur le client (donc sans faire de requête au serveur), les données en fonction de la date.
5. Il faut seulement afficher les rangées avec une date égale ou supérieure à la date entrer dans le champ date.
6. Le filtrage s'effectue lorsque l'on click sur le bouton "filtrer"
7. Encore une fois, il y a plusieurs façons possibles de résoudre ce problème, mais c'est une bonne idée d'ajouter un attribut class sur le **\<td\>** qui contient le date de la certification pour facilement les obtenir dans votre code JavaScript.
...
Vous pouvez simplement utiliser l'attribut **hidden** pour cacher un élément. Il est d'ailleur tout à fait correcte de faire **.setAttribute("hidden", "");**
