...
3. Lorsqu'on ajoute des caractères et qu'on les **retire tous**, le surlignage **doit se retirer de toutes les lignes**.  
...
2. Il faut seulement afficher les rangées avec **une date égale ou supérieure à la date entrée dans le champ date**.
3. Le filtrage s'effectue lorsque l'on **clique sur le bouton "filtrer"**.
...
1. Modifiez le code pour que toutes les pages disponibles soient affichées dans la pagination. Assurez-vous que le `li` de la page sélectionnée ait la bonne classe.
