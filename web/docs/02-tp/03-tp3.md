...
Le nom de la fonction à coder se nomme highlightByName(). La balise input contient déjà le code nécessaire pour appeler cette fonction
...
- La première contient l'**index de la page actuellement sélectionnée** : `PageIndex`. 
   (**Notez que cette propriété contient la même valeur que `selectedPageIndex`** qui est affiché dans le formulaire. Vous pouvez donc utiliser une ou l'autre pour gérer la pagination. Nous rendrons toutefois le input selectedPageIndex hidden plus tard, pour le moment il est visible uniquement pour les tests).
...
2. Créez un ViewModel (**RecordViewModel**) qui contiendra le **Record** ainsi que les **SelectList** correspondants aux éléments `<select>` des formulaires dans les vues. Remplacez l'utilisation du ViewBag par ce ViewModel.
...
5. Créez un **service** pour gérer la création de ViewModels et les interactions avec le DbContext. **<span style={{color: "red"}}>Attention</span>, pour avoir tous les points vous devez utiliser la classe générique `ServiceBaseEF<>`**.
6. Utilisez le **service** dans le contrôleur (Une fois que vous avez terminé, le contrôleur ne devra plus utiliser directement JulieProDBContext)
