...
2. Implémentez la méthode **GetAllAsync** du service **TrainerService** pour prendre en compte les paramètres de pagination de **TrainerSearchViewModelFilter**
...
1. Il faut maintenant utiliser **Items** du modèle **TrainerSearchViewModel** qui est un **IPaginatedList\<Trainer\>** pour bien afficher les liens de navigation
2. Mettez le code pour que l’ensemble des pages disponibles soient affichées dans la pagination.
3. Ajoutez la logique pour que **Previous** et **Next** fonctionnent et s'affichent lorsque nécessaire. 

|![Image Reference](/tps/tp3/navigation.png)|
|-|

4. Ajoutez le javascript en utilisant **jQuery** pour que la page change dans le filtre du formulaire et qu'il se soumette automatiquement lorsqu'on clique sur un élément de pagination (incluant Previous et Next). Utilisez les attributs **data-page-id** qui sont déjà là sur les éléments de navigation. Il y a plusieurs façons d'obtenir le même résultat, mais voici les grandes étapes nécessaires en JS :
   - Exécuter une fonction JS lorsqu'un élément de pagination est cliqué.
   - Obtenir la valeur du **data-page-id** de l'élément sur lequel l'utilisateur a cliqué.
   - Obtenir l'élément qui contient l'information du **SelectedPageIndex** (libre à vous d'ajouter un Id pour vous aider à l'obtenir plus facilement)
